...
if [ ! -f ./Output/usfmsetupconsole.exe ]; then exit;
fi

export GOPATH=$(pwd)
export GITHUB_USER=WycliffeAssociates
export GITHUB_TOKEN=$1
export GITHUB_REPO=quick_usfm_converter_console
...
DOES_RELEASE_EXIST=$(curl -s https://api.github.com/repos/wycliffeassociates/quick_usfm_converter_console/releases/tags/$TAG | grep '"message": "Not Found"')
...
#Upload asset
gothub upload --tag $TAG --name "usfmsetupconsole.exe" --file ./Output/usfmsetupconsole.exe
